...
bot.command("s", async (ctx) => {
  ...
  const following = await Following.find({ chatId, packageName: { $regex: q, $options: 'i' } });
  const packageNames = following.map(item => item.packageName);
  const packages =  await Package.find({ name: {$in: packageNames} });
  if (packages.length === 0) {
    ctx.reply(`No packages found`);
    return;
  }

  ctx.reply(`List of following packages contain ${q}: \n\n${formatListResponse(packages)}`);
});
